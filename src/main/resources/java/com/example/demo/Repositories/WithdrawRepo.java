...
public interface WithdrawRepo extends CrudRepository<WithdrawEntity, Integer> {
    ...
    @Query(value = "select * from  withdraw_entity where user_foreign_key = ?1 and inp_date like ?2", nativeQuery = true)
    List<WithdrawEntity> getAssetsForMonth(int id, String month);
}
