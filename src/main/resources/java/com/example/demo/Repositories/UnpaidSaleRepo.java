...
public interface UnpaidSaleRepo extends CrudRepository<UnpaidSaleEntity, Integer> {
    ...
    @Query(value = "select * from  unpaid_sale_entity where user_foreign_key = ?1 and inp_date like ?2", nativeQuery = true)
    List<UnpaidSaleEntity> getAssetsForMonth(int id, String month);
}
