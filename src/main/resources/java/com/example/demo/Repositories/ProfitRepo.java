...
public interface ProfitRepo extends CrudRepository<ProfitEntity, Integer> {
    ...
    @Query(value = "select * from  profit_entity where user_foreign_key = ?1 and inp_date like ?2", nativeQuery = true)
    List<ProfitEntity> getAssetsForMonth(int id, String month);
}
